...
class DenseBlock(nn.Module):
    def __init__(self, input_size, depth=5, in_channels=256):
...
class Net(nn.Module):
    def __init__(self, L=512, width=256):
        ...
        self.pcaps = PrimaryCaps(256, 256, 9, 4, 64)

        self.dual_transformer = Dual_Transformer(64, 64, nhead=4, num_layers=6)

        self.linear = Linear(8192, 8192)
    ...
    def forward(self, x):
        ...
        shape = out.shape
        out = out.contiguous().view(shape[0], shape[1], shape[2] * shape[3])
        out = self.linear(out)
        out = out.contiguous().view(shape[0], shape[1], shape[2]*4, -1)
        out = torch.permute(out, (0, 2, 1, 3))
